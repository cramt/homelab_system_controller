use dioxus::prelude::*;

#[component]
pub fn Bazarr() -> Element {
    rsx! {
        svg { xmlns: "http://www.w3.org/2000/svg", "viewBox": "0 0 48 48",
            defs {
                clipPath { id: "bazarr_svg__a",
                    path { d: "M0 512h512V0H0z" }
                }
            }
            g {
                transform: "matrix(.09375 0 0 -.09375 0 48.02)",
                "clip-path": "url(#bazarr_svg__a)",
                path {
                    fill: "#fff",
                    d: "M506 256C506 117.93 394.07 6 256 6S6 117.93 6 256s111.93 250 250 250 250-111.93 250-250",
                }
                path {
                    d: "M506 256C506 117.93 394.07 6 256 6S6 117.93 6 256s111.93 250 250 250 250-111.93 250-250z",
                    "stroke-width": "12",
                    "stroke-miterlimit": "10",
                    stroke: "#000",
                    fill: "none",
                }
                path { d: "M406.2 119.47a2368.212 2368.212 0 0 0-300.82 0c-25.418 1.747-50.24 24.551-53.31 50.048-6.847 60.087-6.847 120.17 0 180.26 3.07 25.496 27.892 48.3 53.31 50.048a2368.212 2368.212 0 0 0 300.82 0c25.419-1.748 50.24-24.552 53.311-50.048 6.846-60.087 6.846-120.17 0-180.26-3.071-25.497-27.893-48.3-53.311-50.048" }
                path {
                    fill: "#fff",
                    d: "M348.36 145.31H163.22c-5.452 0-9.914 4.461-9.914 9.913v.001c0 5.452 4.462 9.913 9.914 9.913h185.14c5.452 0 9.913-4.461 9.913-9.913v-.001c0-5.452-4.461-9.913-9.913-9.913m48.18 39.81H115.049c-5.5 0-9.999 4.5-9.999 9.999 0 5.5 4.499 9.999 9.999 9.999H396.54c5.5 0 9.999-4.499 9.999-9.999 0-5.499-4.499-9.999-9.999-9.999",
                }
            }
        }
    }
}
